...
def generateSamples(n, d, p=1, zipfA=1.5):
    ...
    for a in range(n):
        wghts = np.random.zipf(zipfA, size=(d)).astype(float).tolist()
        results['Functions'].append(lambda e, weights=wghts, a=d: sum([(0 if e[x] == None else (abs(e[x])**p)*weights[x]) for x in range(a)]))
        results['Weights'].append(wghts)
    return results
